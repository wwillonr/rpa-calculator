...
**Security & Utilities:**
- **Security:** Helmet, CORS (allowing multiple local instances like `http://localhost:5173` and `http://localhost:5174`)
- **Logging:** Morgan
...
*   **`components/`**: UI Building Blocks.
    ...
    *   **`Settings.jsx`**: Interface for configuring global assumptions (Developer rates, License costs) and managing user administration - acting as an Admin panel. Rendered with multiple tabs.
    *   **`AnnualCostEstimateTab.jsx`**: Component rendered inside Settings to encapsulate UI, state, mathematical logic, and mapping for projecting annual RPA license costs based on operational contexts (24h/12h profiles). Calculates and persists metrics (like average price/cost per robot per year) directly to Firestore via `settingsService`.
...
*   **`controllers/`**: Request Handling.
    ...
    *   **`settingsController.js`**: Manages global configuration (Rates, Baselines, Annual Estimates defaults) stored in Firestore.
...
*   **Global Configuration**: The system relies on a `settings/global_config` document in Firestore. If missing, `calculationService` uses a hardcoded fallback with robust error handling (e.g., fallbacks for empty team composition to avoid 500 errors). This allows Admins to tune costs (e.g., "Developer Hour Cost", "Annual Estimate Inputs", "Default Project Margins") without redeploying code, enabling recalculation logic directly on the UI (e.g., `AnnualCostEstimateTab.jsx`).
